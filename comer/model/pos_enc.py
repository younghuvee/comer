...
class WordPosEnc(pl.LightningModule):
    def __init__(
        ...
        # inv_freq = torch.einsum("i, j -> i j", position, div_term)         #爱因斯坦求和约定，列*行=矩阵，

        # replace1
        position = position.unsqueeze(1)
        div_term = div_term.unsqueeze(0)
        inv_freq = torch.matmul(position, div_term)

        # replace2
        # position = position.unsqueeze(0)
        # div_term = div_term.unsqueeze(1)
        # inv_freq = position * div_term
        # inv_freq = torch.sum(inv_freq, dim = -1)

        pe[:, 0::2] = inv_freq.sin()    #0，2，4...偶数列
        pe[:, 1::2] = inv_freq.cos()
...
class ImgPosEnc(pl.LightningModule):
    ...
    def forward(self, x: torch.Tensor, mask: torch.LongTensor) -> torch.Tensor:
        ...
        # replace1
        x_embed = x_embed.unsqueeze(3)
        inv_feq = inv_feq.unsqueeze(0)
        pos_x = torch.matmul(x_embed, inv_feq)
        y_embed = y_embed.unsqueeze(3)
        pos_y = torch.matmul(y_embed, inv_feq)
...
class WordRotaryEmbed(pl.LightningModule):
    ...
    def forward(self, x: torch.FloatTensor):
        ...
        # replace
        t = t.unsqueeze(1)
        self.inv_freq = self.inv_freq.unsqueeze(0)
        sinusoid_inp = torch.matmul(t, self.inv_freq)
...
class ImageRotaryEmbed(pl.LightningModule):
    ...
    def forward(self, x: torch.Tensor, mask: torch.LongTensor) -> torch.Tensor:
        ...
        # replace
        embed_x = embed_x.unsqueeze(3)
        inv_feq = inv_feq.unsqueeze(0)
        pos_x = torch.matmul(embed_x, inv_feq)
        embed_y = embed_y.unsqueeze(3)
        pos_y = torch.matmul(embed_y, inv_feq)
